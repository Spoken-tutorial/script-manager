...
from creation.models import ContributorRole, FossCategory, Language, TutorialDetail, Level
...
class TestScriptmanager(APITestCase):
    ...
    def setUp(self):
        ...
        ContributorRole.objects.create(foss_category=self.foss1, language=self.lang1, user=test1, status=True)
        ContributorRole.objects.create(foss_category=foss2, language=lang2, user=test2, status=True)
        ContributorRole.objects.create(foss_category=foss3, language=lang3, user=test3, status=True)

        level = Level.objects.create(level="test level", code="TL")

        tutorial1 = TutorialDetail.objects.create(foss=self.foss1, tutorial="test data 1", level=level, order=1, user=admin)
        TutorialDetail.objects.create(foss=self.foss1, tutorial="test data 2", level=level, order=2, user=admin)

        self.client.login(username='test1', password='Test@123')

        self.scripts_url = "/scripts/api/tutorial/" + str(tutorial1.pk) + "/language/" + str(self.lang1.pk) + "/scripts/"
    ...
    def test_revert_reversions(self):
        ...
        self.client.patch(reversion_url + str(reversion.data[1]['reversion_id']) + "/")
        scripts = self.client.get(self.scripts_url)
        self.assertEqual(scripts.data[0]['cue'], "test")
        self.assertEqual(scripts.data[0]['narration'], "test")
